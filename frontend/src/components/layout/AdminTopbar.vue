  ...
  <header class="h-[72px] bg-white border-b border-[#E8E6DE] sticky top-0 z-30 flex items-center justify-between px-5 sm:px-6 lg:px-8">
    ...
    <div class="flex items-center gap-4">
      ...
      <button class="relative p-2 rounded-full text-[#66706C] hover:text-[#173B35] hover:bg-[#F7F5EF] transition-colors">
        <Bell class="w-5 h-5" />
        <span class="absolute top-1.5 right-1.5 w-2 h-2 bg-[#C9965B] rounded-full border border-white"></span>
      </button>
      ...
      <div class="relative">
        ...
        >
          ...
          <div class="hidden sm:block text-left">
            <p class="text-sm font-bold text-[#1D2724] leading-tight">{{ authStore.user?.name || 'Admin' }}</p>
            <p class="text-[10px] font-medium text-[#66706C] leading-tight">Admin</p>
          </div>
        </button>
        ...
        >
          <div class="px-4 py-3 border-b border-[#E8E6DE] sm:hidden">
            <p class="text-sm font-bold text-[#1D2724] truncate">{{ authStore.user?.name || 'Admin' }}</p>
            <p class="text-xs text-[#66706C] truncate">{{ authStore.user?.email || 'admin@example.com' }}</p>
          </div>
          <router-link 
            to="/admin/settings" 
