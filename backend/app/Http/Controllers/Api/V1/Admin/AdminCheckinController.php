...
class AdminCheckinController extends Controller
{
    ...
    {
        $query = ScanLog::with(['order.items.ticketType'])->orderByDesc('created_at');
        if ($s = $request->query('search')) {
            $query->where('order_code','ilike',"%{$s}%");
        }
        if ($perPage = $request->query('per_page')) {
            $p = $query->paginate((int)$perPage);
            $mapped = $p->getCollection()->map(function ($log) {
                return [
                    'id' => $log->id,
            ...
            return response()->json(['success'=>true,'message'=>'Monitoring check-in berhasil diambil','data'=>$mapped,'meta'=>['current_page'=>$p->currentPage(),'last_page'=>$p->lastPage(),'per_page'=>$p->perPage(),'total'=>$p->total()]]);
        }
        ...
        return response()->json(['success'=>true,'message'=>'Monitoring check-in berhasil diambil','data'=>$logs]);
    }
}
